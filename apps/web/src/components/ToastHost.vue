...
import { dismissToast, toastState } from '@/views/mail/compat'
</script>
    ...
    <div class="toast-host" aria-live="polite" aria-atomic="false">
      <TransitionGroup name="toast">
        <div v-for="item in toastState.items" :key="item.id" class="app-toast" :class="item.tone" :role="item.tone === 'error' ? 'alert' : 'status'">
          ...
          <CircleAlert v-else-if="item.tone === 'error' || item.tone === 'warning'" :size="18" />
          <Info v-else :size="18" />
...
.app-toast > button { display: grid; width: 28px; height: 28px; color: var(--muted); background: transparent; border: 0; border-radius: 5px; place-items: center; }
