...
import { Check, ChevronLeft, ChevronRight, LoaderCircle, Network, Plus, Search, ShieldCheck, Trash2, UsersRound } from 'lucide-vue-next'
...
<template>
  <section class="page mail-accounts">
    ...
    <p v-if="error && mailAccountState.accounts.length" class="message error account-message">{{ error }}</p>
...
@media (max-width: 420px) {
  .mail-accounts td[data-label="运行状态"] { grid-template-columns: minmax(0, 1fr); }
  .health-detail { max-width: 100%; white-space: normal; overflow-wrap: anywhere; }
