...
import { ChevronLeft, ChevronRight, CircleOff, Clock3, Copy, Download, LoaderCircle, MailCheck, MailOpen, MailPlus, Pencil, Play, Power, RefreshCw, Search, Trash2 } from 'lucide-vue-next'
...
async function exportCSV() {
  error.value = ''
  try {
    const response = await fetch('/api/v1/mail/aliases/export.csv', {
  ...
  <section class="page mail-aliases">
    ...
    <section class="mail-workspace">
      ...
      <div v-if="workspaceView === 'aliases'" class="aliases-pane">
        <p v-if="error && aliases.length" class="message error pane-message">{{ error }}</p>
