...
export async function loadMailAccounts() {
  const accounts = await mailRequest<MailAccount[]>('/mail/accounts')
...
export async function createMailAccount(name: string) {
  const account = await mailRequest<MailAccount>('/mail/accounts', { method: 'POST', data: { name } })
...
export async function deleteMailAccount(id: string) {
  await mailRequest<{ deleted: boolean; id: string }>(`/mail/accounts/${encodeURIComponent(id)}`, { method: 'DELETE' })
...
export async function updateMailAccountProxy(id: string, proxy: string) {
	const account = await mailRequest<MailAccount>(`/mail/accounts/${encodeURIComponent(id)}/proxy`, {
		method: 'PUT', data: { proxy },
	})
...
export function testMailAccountProxy(id: string, proxy: string) {
	return mailRequest<MailAccountProxyTest>(`/mail/accounts/${encodeURIComponent(id)}/proxy/test`, {
		method: 'POST', data: { proxy },
	})
}
