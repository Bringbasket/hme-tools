...
func TestNewSharedUsesMailRedisLockDefaults(t *testing.T) {
	service := NewShared(nil, nil)
	if service.RedisPrefix() != "gokeep:mail:" {
		t.Fatalf("unexpected shared Redis prefix: %q", service.RedisPrefix())
	}
	if service.config.LockTTL != 10*time.Minute {
		t.Fatalf("unexpected shared lock TTL: %s", service.config.LockTTL)
	}
}
