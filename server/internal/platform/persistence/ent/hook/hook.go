...
// Mutate calls f(ctx, m).
func (f MailboxHiddenMessageFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.MailboxHiddenMessageMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MailboxHiddenMessageMutation", m)
}
...
// The MailboxSyncStateFunc type is an adapter to allow the use of ordinary
// function as MailboxSyncState mutator.
type MailboxSyncStateFunc func(context.Context, *ent.MailboxSyncStateMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f MailboxSyncStateFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.MailboxSyncStateMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MailboxSyncStateMutation", m)
}
