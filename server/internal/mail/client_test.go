...
func TestClientDoesNotExposePlaintextUpstreamBody(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusBadGateway)
		_, _ = w.Write([]byte("Cookie: SESSION=secret-cookie-value\nmessage body with password=secret-password"))
	}))
	defer server.Close()
	client, _ := NewClient(testConfig(), WithBaseURL(server.URL), WithHTTPClient(server.Client()))

	_, err := client.SetAliasActive(context.Background(), "id-1", true)
	if err == nil || strings.Contains(err.Error(), "secret-cookie-value") || strings.Contains(err.Error(), "secret-password") {
		t.Fatalf("upstream plaintext leaked through error: %v", err)
	}
}
