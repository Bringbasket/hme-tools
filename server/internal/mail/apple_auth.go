...
func (session *appleAuthSession) srpHeaders() map[string]string {
	...
	headers := map[string]string{"Accept": "application/json", "Content-Type": "application/json", "Origin": strings.TrimSuffix(session.Endpoints.Auth, "/appleauth/auth"), "Referer": strings.TrimSuffix(session.Endpoints.Auth, "/appleauth/auth") + "/", "X-Apple-Widget-Key": session.ClientID, "X-Apple-App-Id": session.ClientID, "X-Apple-OAuth-Client-Id": session.ClientID, "X-Apple-OAuth-Client-Type": "firstPartyAuth", "X-Apple-OAuth-Redirect-URI": session.Endpoints.Home, "X-Apple-OAuth-Require-Grant-Code": "true", "X-Apple-OAuth-Response-Mode": "web_message", "X-Apple-OAuth-Response-Type": "code", "X-Apple-OAuth-State": frame, "X-Apple-Frame-Id": frame, "X-Requested-With": "XMLHttpRequest", "X-Apple-Mandate-Security-Upgrade": "0", "X-Apple-I-Require-UE": "true", "X-Apple-I-FD-Client-Info": fdClientInfo}
	if session.Scnt != "" {
