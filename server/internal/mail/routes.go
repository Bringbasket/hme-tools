...
import (
	...
	"gokeep/server/internal/platform/activitylog"
...
func (api *routeAPI) shareRemoved(w http.ResponseWriter, _ *http.Request) {
	shareJSON(w, http.StatusGone, map[string]string{"error": "旧版 /share 分享地址已停用，请使用 /share/v1/latest?email=...&token=..."})
}
...
func (api *routeAPI) createBatchShareLinks(w http.ResponseWriter, r *http.Request) {
	const (
		...
		batchShareScopeGPTUnregistered = "gpt_unregistered"
	)
	payload := struct {
	...
	if payload.Scope != batchShareScopeAll && payload.Scope != batchShareScopeGPTRegistered && payload.Scope != batchShareScopeGPTUnregistered {
		httpx.WriteError(w, r, http.StatusBadRequest, "BAD_REQUEST", "scope 必须是 all、gpt_registered 或 gpt_unregistered")
		return
	}
	aliases, err := api.sessionFor(r).ListAliases(r.Context())
	if err != nil {
	...
	if payload.Scope != batchShareScopeAll {
	...
	for index, raw := range aliases {
		...
		registeredGPT := aliasHasRegisteredGPT(raw)
		if payload.Scope == batchShareScopeGPTRegistered && !registeredGPT {
			continue
		}
		if payload.Scope == batchShareScopeGPTUnregistered && registeredGPT {
			continue
		}
	...
	if payload.Count > len(candidates) {
		...
		} else if payload.Scope == batchShareScopeGPTUnregistered {
			description = "未注册 GPT 的启用邮箱"
		}
...
func (api *routeAPI) writeMailError(w http.ResponseWriter, r *http.Request, err error) {
	...
	if errors.As(err, &upstream) || errors.As(err, &apple) {
		httpx.WriteError(w, r, http.StatusBadGateway, "ICLOUD_ERROR", safeErrorText(err))
		return
	}
