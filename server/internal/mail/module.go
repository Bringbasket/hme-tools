...
import (
	...
	"gokeep/server/internal/platform/storage"
)
...
// RegisterRoutesAt registers the canonical management API below a host-owned
// prefix. GoKeep uses /api/v1 for all authenticated APIs, while the mail
// module keeps the same standard-library routing and response behavior.
func (module *Module) RegisterRoutesAt(mux *http.ServeMux, auth httpx.Middleware, base string) {
	base = strings.TrimRight(strings.TrimSpace(base), "/")
	if base == "" {
		base = "/api/mail/v1"
	}
	(&routeAPI{module: module}).register(mux, auth, base, false)
}
